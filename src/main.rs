...
async fn handle_deploy(
    ...
    instance_client.invoke(instance_name, template_id, function, InvokeParameters{params: parameters}, auth).await?;

    let res = DeployResult { template, instance};
...
async fn async_main(cmd: GolemCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let instance_srv = InstanceHandlerLive { client: instance_client.clone(), templates: &template_srv };

    let auth = auth_srv.authenticate(cmd.auth_token.clone(), cmd.config_directory.clone().unwrap_or(default_conf_dir)).await?;

    let res = match cmd.command {
        Command::Deploy { project_ref, template_name, instance_name, env, function, parameters, template_file, args } => {
            handle_deploy(&project_client, &template_client, &instance_client, &auth, project_ref, template_name, instance_name, env, function, parameters, template_file, args).await
        }
