...
pub trait InstanceClient {
    ...
    async fn get_invocation_key(&self, name: &InstanceName, template_id: &RawTemplateId, auth: &CloudAuthentication) -> Result<InvocationKey, GolemError>;
    ...
    async fn interrupt(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<(), GolemError>;
    async fn simulated_crash(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<(), GolemError>;
    async fn delete(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<(), GolemError>;
    async fn get_metadata(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<InstanceMetadata, GolemError>;
    async fn connect(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<(), GolemError>;
}
...
impl<C: golem_client::instance::Instance + Send + Sync> InstanceClient for InstantClientLive<C> {
    async fn new_instance(&self, name: InstanceName, template_id: RawTemplateId, args: Vec<String>, env: Vec<(String, String)>, auth: &CloudAuthentication) -> Result<ComponentInstance, GolemError> {
        info!("Creating instance {name} of {}", template_id.0);
    ...
    async fn invoke_and_await(&self, name: InstanceName, template_id: RawTemplateId, function: String, parameters: InvokeParameters, invocation_key: InvocationKey, use_stdio: bool, auth: &CloudAuthentication) -> Result<InvokeResult, GolemError> {
    ...
    async fn invoke(&self, name: InstanceName, template_id: RawTemplateId, function: String, parameters: InvokeParameters, auth: &CloudAuthentication) -> Result<(), GolemError> {
    ...
    async fn delete(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<(), GolemError> {
        info!("Deleting instance {}/{}", template_id.0, name.0);
    ...
    async fn get_metadata(&self, name: InstanceName, template_id: RawTemplateId, auth: &CloudAuthentication) -> Result<InstanceMetadata, GolemError> {
        info!("Getting instance {}/{} metadata", template_id.0, name.0);
