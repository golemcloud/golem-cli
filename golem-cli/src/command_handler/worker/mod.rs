...
use crate::model::text::worker::{FileNodeView, WorkerFilesView};
...
impl WorkerCommandHandler {
    ...
    async fn cmd_files(&self, worker_name: WorkerNameArg, path: String) -> anyhow::Result<()> {
        ...
        // Convert nodes to WorkerFilesView
    ...
    async fn cmd_file_contents(
        ...
        match File::create(&output_path).and_then(|mut file| file.write_all(&file_contents)) {
            Ok(_) => {
                log_action("File saved", format!("to {output_path}"));
                Ok(())
            }
            Err(e) => {
                log_warn_action("Failed to save file", format!("to {output_path}: {e}"));
                Err(e.into())
            }
        }
    }
