...
mod mcp_server {
    use super::*;
    use rust_mcp_sdk::mcp_server::prelude::*;
    use rust_mcp_sdk::mcp_server::hyper_server;
    use rust_mcp_sdk::schema::{Resource, Tool};
    use serde_json::json;
    ...
    pub async fn run(ctx: Arc<Context>, port: u16) -> anyhow::Result<()> {
        ...
        // Start SSE server via hyper on the given port
        let addr = ([127, 0, 0, 1], port).into();
        hyper_server::create_server(server.build(handler)?, addr)
            .await
