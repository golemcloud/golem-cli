...
impl FromStr for ComponentReference {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match segments.len() {
            1 => Ok(Self::JustName(segments.pop().unwrap().to_string())),
            2..3 => {
                let component_name = segments.pop().unwrap().to_string();
                let project = ProjectReference::from_str(segments.pop().unwrap())?;

                Ok(Self::WithProject { component_name, project })
            }
            _ => Err(format!("Unknown format for component: {s}. Expected either {{component_name}} or {{project_name}}/{{component_name}} or {{account_email}}/{{project_name}}/{{component_name}}."))
        }
    }
}
...
impl Display for ComponentReference {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::JustName(component_name) => write!(f, "{}", component_name),
            Self::WithProject {
                project,
                component_name,
            } => write!(f, "{}/{}", project, component_name),
        }
    }
}
...
impl FromStr for WorkerReference {
    type Err = String;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match segments.len() {
            1 => Ok(Self::JustName(segments.pop().unwrap().to_string())),
            2..4 => {
                let worker_name = segments.pop().unwrap().to_string();
                let component = ComponentReference::from_str(segments.pop().unwrap())?;

                Ok(Self::WithComponent { worker_name, component })
            }
            _ => Err(format!("Unknown format for worker: {s}. Expected either {{worker_name}} or {{component_name}}/{{worker_name}} or {{project_name}}/{{component_name}}/{{worker_name}} or {{account_email}}/{{project_name}}/{{component_name}}/{{worker_name}}."))
        }
    }
}

impl Display for WorkerReference {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::JustName(worker_name) => write!(f, "{}", worker_name),
            Self::WithComponent {
                component,
                worker_name,
            } => write!(f, "{}/{}", component, worker_name),
        }
    }
}
