...
describe('WSS (WebSocket Service)', () => {
  ...
  describe('getConnection', () => {
    it('should create WSS connection with localhost URL', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881/api/websocket');
    ...
    it('should replace http with ws in URL', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881/test');
    });

    it('should handle connection failures', async () => {
      const { UniversalWebSocket } = await import('@/lib/tauri&web.ts');
      (UniversalWebSocket.connect as any).mockRejectedValue(new Error('Connection failed'));

      await expect(WSS.getConnection('/fail')).rejects.toThrow('Connection failed');
    });
    ...
    it('should use fallback when fetchCurrentIP returns null', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881/fallback');
    });
  });
  ...
  describe('WebSocket operations', () => {
    ...
    describe('onMessage', () => {
      ...
      it('should pass through callback parameters correctly', () => {
        ...
        expect(mockWebSocket.onMessage).toHaveBeenCalledWith(expect.any(Function));
  ...
  describe('URL construction', () => {
    it('should handle paths with leading slash', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881/api/stream');
    });
    ...
    it('should handle paths without leading slash', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881api/stream');
    });
    ...
    it('should handle empty path', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881');
    });
    ...
    it('should handle complex URLs with query parameters', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881/api/stream?token=abc123&channel=main');
    });
  });
  ...
  describe('Error handling', () => {
    it('should propagate connection errors', async () => {
      ...
      await expect(WSS.getConnection('/error')).rejects.toThrow('WebSocket connection failed');
    });
  ...
  describe('Integration scenarios', () => {
    it('should support full websocket lifecycle', async () => {
      ...
      expect(UniversalWebSocket.connect).toHaveBeenCalledWith('ws://localhost:9881/lifecycle');
      expect(mockWebSocket.onMessage).toHaveBeenCalledWith(messageHandler);
