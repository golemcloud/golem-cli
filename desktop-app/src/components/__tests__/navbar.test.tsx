...
describe('Navbar', () => {
    ...
    it('hides navigation links when showNav is false', () => {
        ...
        expect(screen.queryByTestId('nav-link-deployments')).not.toBeInTheDocument();
    ...
    it('generates correct URLs for navigation links with appId', () => {
        ...
        expect(screen.getByTestId('nav-link-dashboard')).toHaveAttribute('href', `/app/${appId}/dashboard`);
        expect(screen.getByTestId('nav-link-components')).toHaveAttribute('href', `/app/${appId}/components`);
        expect(screen.getByTestId('nav-link-apis')).toHaveAttribute('href', `/app/${appId}/apis`);
        expect(screen.getByTestId('nav-link-deployments')).toHaveAttribute('href', `/app/${appId}/deployments`);
        expect(screen.getByTestId('nav-link-plugins')).toHaveAttribute('href', `/app/${appId}/plugins`);
    });
    ...
    it('has correct CSS classes for layout', () => {
        ...
        expect(container).toHaveClass('flex', 'items-center', 'justify-between', 'px-4', 'py-2');
    });
