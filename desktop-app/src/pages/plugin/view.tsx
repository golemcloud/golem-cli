...
import API.deletePluginimport plugin from ".";
(appId!, currentVersion.name, currentVersion.version)
  .then(res => {
    if (plugin.length > 1) {
      navigate(`/app/${appId}/plugins/${plugin[0].name}`);
    } else {
      navigate(`/app/${appId}/plugins`);in } from "@/types/plugin";
              ...
              <CardHeader className="p-4">
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    <Button variant="link" onClick={() => navigate(`/app/${appId}/plugins`)}>
