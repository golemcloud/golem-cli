...
describe('Home Page', () => {
  ...
  describe('Initial rendering', () => {
    ...
    it('should render action cards', async () => {
      ...
      expect(screen.getByRole('heading', { name: 'Open Existing Application' })).toBeInTheDocument();
      expect(screen.getByText('Start a new Golem application project')).toBeInTheDocument();
      expect(screen.getByText('Open and work with an existing Golem application')).toBeInTheDocument();
    });
  ...
  describe('Opening existing app', () => {
    it('should handle successful app opening', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockNavigate).toHaveBeenCalledWith(expect.stringMatching(/^\/app\/app-\d+$/));
      });
    });
    ...
    it('should show loading state during app opening', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Open Existing Application')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('App search functionality', () => {
    ...
    it('should show no matching message when search has no results', async () => {
      ...
      expect(screen.getByText('No matching applications found')).toBeInTheDocument();
    });
