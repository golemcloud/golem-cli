...
vi.mock('@/pages/dashboard/componentSection', () => ({
  ComponentsSection: () => <div data-testid="components-section">Components Section</div>,
}));
...
vi.mock('@/pages/dashboard/deploymentSection', () => ({
  DeploymentSection: () => <div data-testid="deployment-section">Deployment Section</div>,
}));
...
describe('Dashboard', () => {
  ...
  describe('Data Loading', () => {
    ...
    it('should update app last opened on mount', async () => {
      ...
      await waitFor(() => {
        expect(storeService.updateAppLastOpened).toHaveBeenCalledWith('test-app-id');
      });
    });

    it('should handle app loading errors gracefully', async () => {
      const { storeService } = await import('@/lib/settings');
      (storeService.getAppById as any).mockRejectedValue(new Error('Failed to load app'));
  ...
  describe('Layout', () => {
    it('should have proper grid layout structure', async () => {
      ...
      await waitFor(() => {
        const gridElement = screen.getByTestId('components-section').parentElement;
  ...
  describe('Accessibility', () => {
    it('should have proper heading structure', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('heading', { name: /Working in Test App/i })).toBeInTheDocument();
      });
    });
