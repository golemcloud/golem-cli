...
vi.mock('@/components/ui/dialog', () => ({
  Dialog: ({ children, open, onOpenChange }: any) => (
    <div>
      <div data-trigger onClick={() => onOpenChange?.(true)}>{children[0]}</div>
...
vi.mock('@/components/ui/button', () => ({
  Button: ({ children, onClick, variant, disabled, asChild, ...props }: any) => {
    if (asChild) {
...
describe('DeploymentList', () => {
  ...
  describe('Deployment Actions', () => {
    it('should handle create deployment button click', async () => {
      ...
      expect(mockNavigate).toHaveBeenCalledWith('/app/test-app-id/deployments/create');
    });
    ...
    it('should handle delete deployment action', async () => {
      ...
      const deleteButton = screen.getAllByTestId('trash-icon')[0].closest('div');
      ...
      expect(API.deleteDeployment).toHaveBeenCalledWith('test-app-id', 'localhost:8080');
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle delete deployment errors', async () => {
      const { API } = await import('@/service');
      (API.deleteDeployment as any).mockRejectedValue(new Error('Delete failed'));
  ...
  describe('Loading States', () => {
    it('should show loading state while fetching data', async () => {
      const { API } = await import('@/service');
      (API.getApiList as any).mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
      (API.getDeploymentApi as any).mockResolvedValue([]);
